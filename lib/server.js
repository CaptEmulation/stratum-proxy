...
var miningAuthorized = function (client, serverModel, target, defer) {
   return function (str) {
      ...
      var data = parseJson(str), authorizationResponse;
      if (data && !defer.isFulfilled && data.method && data.method.indexOf('mining.authorize') !== -1) {
         if (data.params && Array.isArray(data.params) && paramsMatchServerPassword(data, serverModel)) {
            authorizationResponse = JSON.stringify({
               id: data.id,
               error: null,
               "result": true
            }) + '\n';
            console.log('miner authorized--' + authorizationResponse);
            client.write(authorizationResponse);
            defer.resolve();
         } else {
            authorizationResponse = JSON.stringify({
               id: data.id,
            ...
            client.write(authorizationResponse);
            console.log('miner authorize failed--' + authorizationResponse);
            client.end();
            target.end();
            defer.reject();
         }
   ...
   var serverSocket = net.createServer(function (rawSocket) {
      ...
      client.on('data', function (data) {
         ...
         if (authorizedDefer.promise.isFulfilled()) {
            ...
            console.log('Attempting to authorize client.--' + data);
