...
exports.create = function (options) {
  ...
  var target = createTarget({ client: target, socket: middleTargetSocket});
  var server = createServer({ server: server, socket: middleServerSocket});
  connectTargetToServer(target, server).then(function () {
    defer.resolve();
  });
  return defer.promise;
}
